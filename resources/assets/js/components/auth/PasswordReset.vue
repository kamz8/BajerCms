                ...
                <form v-show="!success" class="form-horizontal" method="POST" @submit.prevent="resetPasswort">
                    <div class="form-group" >
                        ...
                        <input :disabled="processing" class="form-control" v-model="email" name="email" id="InputEmail1" type="email"
                               ...
                               required autofocus>
                        <div v-show="failedAuth" class="invalid-feedback">{{error.message}}</div>
                        <small id="emailHelp" class="form-text text-muted" v-show="!failedAuth">Podaj adres email konta, do którego chcesz odzyskać hasło.</small>
                    </div>
